...
class Client:
    ...
    def __init__(self, grpc_channel: grpc.aio.Channel, svc_addr: str) -> None:
        ...
        self._gridpool_orders_stream: dict[
        ...
        self._public_trades_stream: dict[
            PublicTradeFilter,
            GrpcStreamingHelper[
    ...
    async def stream_gridpool_orders(  # pylint: disable=too-many-arguments
        ...
        if stream_key not in self._gridpool_orders_stream:
            self._gridpool_orders_stream[stream_key] = GrpcStreamingHelper(
        ...
        return self._gridpool_orders_stream[stream_key].new_receiver()
    ...
    async def stream_public_trades(
        ...
        if public_trade_filter not in self._public_trades_stream:
            self._public_trades_stream[public_trade_filter] = GrpcStreamingHelper(
        ...
        return self._public_trades_stream[public_trade_filter].new_receiver()
    ...
    async def update_gridpool_order(  # pylint: disable=too-many-arguments, too-many-locals
        ...
        if all(value is self.NO_VALUE for value in params.values()):
            raise ValueError("At least one field to update must be provided.")

        paths = [param for param, value in params.items() if value is not self.NO_VALUE]
        ...
        update_order_fields = UpdateOrder(
            price=params.get("price", None),  # type: ignore
            quantity=params.get("quantity", None),  # type: ignore
            stop_price=params.get("stop_price", None),  # type: ignore
            peak_price_delta=params.get("peak_price_delta", None),  # type: ignore
            display_quantity=params.get("display_quantity", None),  # type: ignore
            execution_option=params.get("execution_option", None),  # type: ignore
            valid_until=params.get("valid_until", None),  # type: ignore
            payload=params.get("payload", None),  # type: ignore
            tag=params.get("tag", None),  # type: ignore
        )
